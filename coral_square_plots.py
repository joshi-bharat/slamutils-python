...
if __name__ == '__main__':
    ...
    folder = '/home/bjoshi/code/slamutils-python/coral_square'
    title = r'$\boldsymbol{coralreef\_square}$'
    ...
    svin_with_clahe_traj = file_interface.read_tum_trajectory_file(
        os.path.join(folder, 'svin_with_clahe.txt'))
    ...
    x = svin_with_clahe_traj.positions_xyz[:, 0]
    y = svin_with_clahe_traj.positions_xyz[:, 1]
    svin_with_clahe_plot, = ax.plot(x, y, '-', color='black')
    ...
    ax.legend([svin_with_clahe_plot, svin_plot, prim_plot, lc, switch_plot], [r'$\bf{svin\_clahe}$', r'$\bf{svin}$',
                                                                              r'$\bf{primitive}$',
                                                                              r'$\bf{robust}$', r'$\bf{switch\_times}$'],
              handler_map={type(lc): HandlerDashedLines()},
              frameon=True, prop={'size': 16}, loc='upper right')

    plt.savefig('coral_square_with_clahe.png', bbox_inches='tight')

    plt.show()

    # svin_gt_traj, robust_traj = sync.associate_trajectories(
    #     svin_with_clahe_traj, prim_traj, 0.1)
